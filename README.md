...
Let's create a project, in this example I'm calling my project of **split-traffic**, you also can set the name of your project in a environment variable called ```PROJECT_ID``` running an export command in your shell/bash:<br>
<pre>
...
https://**VERSION**-dot-**SERVICE**-dot-**PROJECT_ID**.**REGION_ID**.r.appspot.com
 ...
 - **SERVICE**: is the app name, in this case is **python-app**.
 - **PROJECT_ID**: is the project ID where's you activated the app engine resource and deployed the app, I defined my PROJECT_ID in the last [step](#createProject).
 - **REGION_ID**: the app engine is a region resource, iin this case I chose ```us-central``` region.

I will access the app using this URL:

https://python-app-dot-split-traffic.uc.r.appspot.com

![](./img/app-engine-browser.png)

And we can see that's it's working 🤩.
...
The **versions** run on the **instances**, and what I'm going to do is create a new version of the "python-app" service and with this second version deployed, we can divide the traffic so that only a percentage of accesses see the new version
...
To create a new version of our app, let's start editing [main.py](app/main.py) file, and change the message string that was being returned in ```index()``` function, from "App 1" to "App 2".
...
In the command I use to deploy the new version, 2 very important arguments were used, which are:
 
  - **--version**: where we set the name of version.
  - **--no-promote**: this command is for the version we are deploying does not receive any traffic.
...
To run this script you need to give execute permission in your shell, to do this run the command below:
<pre>
sudo chmod +x script.sh
</pre>

Run the script.sh using this syntax:
<pre>
...
Executing this command you will migrate all traffic immediately, but if you want to migrate gradually, add this flag ```--migrate``` in the command.
